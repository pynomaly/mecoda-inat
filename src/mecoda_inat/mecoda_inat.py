...
API_PATH = "https://api.inaturalist.org/v1"
...
def _get_taxon_columns(df_obs: pd.DataFrame):
    file_path = resources.files("mecoda_inat.data") / "taxon_tree.csv"
        ...
        for ancestry in list_ancestries:
            if int(ancestry) != 1:
                try:
                    ...
                    # para rangos intermedios, que no están en el taxon_tree, no los incluye
                    continue
    except:
        data = None
...
def download_photos(df_photos: pd.DataFrame, directorio: Optional[str] = "inat_photos"):
    """
...
def get_dwc(observations: List) -> pd.DataFrame:
    ...
    for id_ob in id_obs:
        url = f"{BASE_URL}/observations.dwc?id={id_ob}"
        df = pd.read_xml(url, parser="etree")
        df_total = pd.concat([df_total, df])

    # clean fields
    df_total["institutionCode"] = "INaturalist"
